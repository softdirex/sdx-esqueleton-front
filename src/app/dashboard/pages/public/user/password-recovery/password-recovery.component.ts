...
export class PasswordRecoveryComponent implements OnInit {
  ...
  sendRequest(email: string) {
    this.recaptchaV3Service
      .execute('esqueleton_password_recovery')
      .subscribe((token: string) => {
        this.reCAPTCHAToken = token;
        this.step = this.STEP_LOADING;
        this.customerService.sendPasswordRecoveryRequest(email).subscribe({
          next: (v) => {
              ...
              switch (e.error.detail) {
                ...
                case 'User not registered in this product':
                  ...
                  this.alertLinkModal.onClose.subscribe((accept: any) => {
                    if (accept) {
                      window.open(
                        environment.coreFrontendEndpoint + 'password-recovery'
                      );
                    }
                  });
                  break;
